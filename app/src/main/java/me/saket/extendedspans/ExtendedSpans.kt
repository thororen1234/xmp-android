...
@Stable
class ExtendedSpans(vararg painters: ExtendedSpanPainter) {
    private val painters = painters.toList()
    internal var drawInstructions = emptyList<SpanDrawInstructions>()

    /**
     * Prepares [text] to be rendered by [painters]. [SquigglyUnderlineSpanPainter]
     * use this for removing background and underline spans so that they can be drawn manually.
     */
    fun extend(text: AnnotatedString): AnnotatedString = buildAnnotatedString {
        append(text.text)
...
fun Modifier.drawBehind(spans: ExtendedSpans): Modifier = drawBehind {
